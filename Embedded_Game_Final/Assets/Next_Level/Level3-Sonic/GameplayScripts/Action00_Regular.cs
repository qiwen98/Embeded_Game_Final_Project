...
public class Action00_Regular : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //Do Spindash
        if ((Input.GetButton("B")|| getButtonBDown) && Player.Grounded)
        {
        ...
        //Play Rolling Sound
        if ((Input.GetButtonDown("R1")|| getButtonR1Down) && Player.Grounded)
        {
        ...
        if (Player.Grounded)
        {
            Vector3 newForward = Player.rigidbody.velocity -
                                 transform.up * Vector3.Dot(Player.rigidbody.velocity, transform.up);
        ...
        //Do a homing attack
        if (!Player.Grounded && (Input.GetButtonDown("A")|| getButtonADown) && Actions.Action02Control.HasTarget &&
