...
public class PlayerMoveQw : MonoBehaviour 
{
    ...
    private void checkplayerhitinput()
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            // if range
            if (EquipmentManager.instance.bow.activeSelf == true)
            {
                animator.Play("Shooting Arrow", -1, 0.5f);
                GameObject bulletGameObject = Instantiate(ArrowPrefab, ShootingPoint.position, Quaternion.identity);
                bulletGameObject.GetComponent<ShootArrow>().Initialize(transform.forward, 40);
                GetComponent<AudioSource>().volume = 1;
                GetComponent<AudioSource>().clip = ShootArrow;
            ...
            if (Physics.Raycast(ray, out hit, 100))
            {
               ...
               // CharacterCombat enemycombat= hit.collider.GetComponent<CharacterCombat>();
                if (interactable != null)
                {
                    Setfocus(interactable);

                    if(characterCombat.attackCooldown<=0)
                    {
                        //if meelee
                        if(EquipmentManager.instance.sword.activeSelf==true)
                        {

                            int random = Random.Range(1, 4);

                            GetComponent<AudioSource>().volume = 1;
                            GetComponent<AudioSource>().clip = SwordSound;
                            GetComponent<AudioSource>().Play();
                            float randomfloat = Random.Range(0.0f, 1f);
                            animator.SetFloat("AttackType", randomfloat);
    ...
    void FixedUpdate() 
	{
		...
		if(animator)
		{
            if(characterMotor.DistanceToTarget>=0.1&&timer>0.2&&IsGrounded())
            {
                RandomAudioPlayer.instance.PlayRandomClip();
                timer = 0;
            }
